...
def create_losing_moves(game: TicTacToe, moves_to_miss):
    ...
    losing_moves = []
    for row in range(game.BOARD_LEN):
        for column in range(game.BOARD_LEN):
...
def do_moves_sequentially(board: TicTacToe, winning_moves, losing_moves, winning_player):
    ...
    for idx, w_move in enumerate(winning_moves):
        board.place_marker(board.PLAYER_TOKENS[losing_player], *losing_moves[idx])
        state = board.place_marker(board.PLAYER_TOKENS[winning_player], *w_move)
    return state
...
def test_winner_diagonal(board_size):
    ...
    assert do_moves_sequentially(board, winning_moves, losing_moves, 0) == TicTacToe.State.NAUGHT_WON


@pytest.mark.parametrize("board_size", BOARD_SIZES)
def test_winner_inverse_diagonal(board_size):
    board = TicTacToe(board_size=board_size)
    winning_moves = create_line_of_moves(board, Position(board.BOARD_LEN - 1, 0), Position(-1, 1))
    losing_moves = create_losing_moves(board, winning_moves)
    assert do_moves_sequentially(board, winning_moves, losing_moves, 1) == TicTacToe.State.CROSS_WON
...
def test_reset(board_size):
    board = TicTacToe(board_size=board_size)
    board.place_marker(TicTacToe.Token.CROSS, 1, 1)
    board.place_marker(TicTacToe.Token.NAUGHT, 0, 0)
    board.reset()
...
def test_draw():
    ...
    board.place_marker(TicTacToe.Token.CROSS, 1, 1)
    board.place_marker(TicTacToe.Token.NAUGHT, 0, 0)
    board.place_marker(TicTacToe.Token.CROSS, 2, 2)
    board.place_marker(TicTacToe.Token.NAUGHT, 0, 1)
    board.place_marker(TicTacToe.Token.CROSS, 1, 0)
    board.place_marker(TicTacToe.Token.NAUGHT, 1, 2)
    board.place_marker(TicTacToe.Token.CROSS, 0, 2)
    board.place_marker(TicTacToe.Token.NAUGHT, 2, 0)
    assert board.place_marker(TicTacToe.Token.CROSS, 2, 1) == TicTacToe.State.DRAW
